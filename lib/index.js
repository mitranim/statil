...
var Statil = (function () {
    ...
    Statil.prototype.render = function (data) {
        ...
        for (var path in this.templates) {
            if (methods.isIgnored.call(this, path))
                continue;
    ...
    Statil.prototype.meta = function (path) {
        ...
        if (path.slice(-1) === '/')
            dirname = path.slice(0, -1);
        else
            dirname = pt.dirname(path);
...
if (process.env.STATIL_ENV === 'testing')
    Statil.methods = methods;
/*-------------------------------- Rendering --------------------------------*/
/**
...
methods.imports = function () {
    ...
    imports.$include = function (path, data) {
        ...
        data = new Hash(data);
        return methods.renderOne.call(_this, path, data);
    };
    /**
 ...
 * Pure hash table.
 */
function Hash(attrs) { _.assign(this, attrs); }
