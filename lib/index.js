 ...
 *
 * Metadata configuration (using pseudosyntax for exposition):
 *
 * ignore: <expression>
 *
 *   Ignores the files whose names match the expression, which is treated as a
 *   regex. Note that #renderAll doesn't have access to file extensions, only
 *   names are checked.
 *
 * files: <filename>: repeat: <key>
 *
 *   Instead of rendering the template, we look for an array of hash tables
 *   under this <key> in the root of its metadata file. For each of those
 *   values, we merge it into a clone of the 'data' argument and pass to a
 *   #render call with the template's path. Each value must be a hash table with
 *   a string under the key 'name', which becomes the filename of the rendered
 *   result.
 *
 *   A template with a valid 'repeat' option is only rendered for the repeated
 *   values. If the array has zero length, it's not rendered at all.
 *
 * @param   Hash   data (optional)
 * @returns String
 */
Statil.prototype.renderAll = function(data) {
  ...
  _.each(this.templates, function(t, path) {
    // Get hold of the directory metadata.
    ...
    if (meta) {
      ...
      if (meta.ignore) {
        ...
        if (typeof meta.ignore !== 'string') {
          throw new TypeError("an 'ignore' expression must be a string")
        }
        // Ignore if the basename matches the ignore expression in meta.
        if (pt.basename(path).match(meta.ignore)) return
      }
      ...
      var options = meta.files && meta.files[name]
      if (options && options.repeat) {
        // Validate the input.
        if (typeof options.repeat !== 'string' || !_.isArray(meta[options.repeat])) {
          throw new TypeError("a 'files[<filename>].repeat' option must be a string key referencing an array in its metadata file")
        }
        // Loop over the elements, rendering the template for each.
        for (var elem of meta[options.repeat]) {
          // Validate the input.
          if (!_.isObject(elem)) {
            throw new TypeError('each element in a repeater list must be a hash table')
          }
          if (typeof elem.name !== 'string' || !elem.name) {
            throw new TypeError('each element in a repeater list must have a non-empty .name value')
          }
          // Clone the locals object and merge it with the element.
          var locals = _.assign(clone(data), elem)
          // Form a new path for the file and assign it to the data object.
          // Because #render and #renderOne are defined to never reassign the
          // $path, it will be available throughout the render chain.
          var pathNew = pt.join(pt.dirname(path), elem.name)
          locals.$path = pathNew
          // Render the template and assign it to the buffer.
          buffer[pathNew] = this.render(path, locals)
        }
        // Exit this iteration of #renderAll, preventing the template from
        // rendering normally.
        return
      }
    }
...
Statil.prototype.meta = function(path) {
  ...
  if (typeof path !== 'string') {
    throw new TypeError('expected a string path, got: ' + path)
  }
...
methods.locals = function(path, data) {
  ...
  if (meta && meta.files && meta.files[basename]) {
    _.assign(data, meta.files[basename])
  }
}
