...
function createSettings(templates, settings) {
  settings = Object(settings)
  const entries = mapVals(templates, toEntry)
  const tree = toTree(entries)
  settings = patch(settings, {
    context: patch(settings.context, {
      templates,
      entries,
      tree,
      render(entry, locals) {
...
function renderEntry(entry, templateSettings, locals) {
  ...
  if (!entry.compiledTemplate) {
    try {
      entry.compiledTemplate = compileTemplate(entry.content, templateSettings)
    }
    catch (err) {
...
function toEntry(content, path) {
  const {attributes, body} = fm(content)

  function isCurrent(subpath, opts) {
    const predicate = opts && opts.exact ? samePath : pathStartsWith
    return predicate(subpath, path)
  }
  ...
  return patch(
    {path, content: body, isCurrent, current, compiledTemplate: undefined},
    attributes
  )
}
...
function isEntry(value) {
  return isObject(value) && 'path' in value && 'content' in value
}
